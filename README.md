...
#### Notes
The IL2P description says the idle flag is supposed to be ```01010101``` during ```txdelay``` and ```txtail``` but this doesn't work for PSK, so we send '00' and '11' as 1200 Baud BPSK, and use ```11001100``` or ```0xCC``` for the flag..   
```
+----------------------+-------------+---------------------+

...
static complex float mPrecedingSample;
static complex float mCurrentSample;
static complex float mMiddleSample;

static float *mDelayLineInphase;
static float *mDelayLineQuadrature;

static float mSamplingPoint;
static float mSampleCounterGain;
static float mDetectedSamplesPerSymbol;
static float mDetectedSamplesPerSymbolGain;
static float mMaximumSamplesPerSymbol;
static float mMinimumSamplesPerSymbol;
static float mGain;
...
void create_interpolatingSampleBuffer(float sampleCounterGain)
{
    ...
    mMinimumSamplesPerSymbol = (float)RATE * (1.0f - MAXIMUM_DEVIATION_SAMPLES_PER_SYMBOL);
...
void destroy_interpolatingSampleBuffer()
{
    free(mDelayLineInphase);
    free(mDelayLineQuadrature);
}
 ...
 */
static float interp_filter(float samples[], int offset, float mu)
{
    // TODO
 ...
 * @param sample
 */
void interp_receive(complex float sample)
{
    mSamplingPoint--;
 ...
 */
void resetAndAdjust(float symbolTimingError)
{
 ...
 */
static float getInphase(float interpolation)
{
    if (interpolation < 0.0f)
    {
        return interp_filter(mDelayLineInphase, mDelayLinePointer, 0.0f);
    }
 ...
 */
static float getQuadrature(float interpolation)
{
    if (interpolation < 0.0f)
    {
        return interp_filter(mDelayLineQuadrature, mDelayLinePointer, 0.0f);
    }
    else if (interpolation < 1.0f)
    {
 ...
 */
complex float getPrecedingSample()
{
    mPrecedingSample = CMPLXF(mDelayLineInphase[mDelayLinePointer + 3], mDelayLineQuadrature[mDelayLinePointer + 3]);
 ...
 */
complex float getCurrentSample()
{
    // Calculate interpolated current sample
    // -10000.0 means there wasn't enough samples yet

    float re = getInphase(mSamplingPoint);
    float im = getQuadrature(mSamplingPoint);
...
{
    float halfDetectedSamplesPerSymbol = mDetectedSamplesPerSymbol / 2.0;

    float re = getInphase(halfDetectedSamplesPerSymbol);
    float im = getQuadrature(halfDetectedSamplesPerSymbol);
